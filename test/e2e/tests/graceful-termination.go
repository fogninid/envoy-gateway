...
func awaitDeploymentAvailable(
	t *testing.T, ctx context.Context,
	gwAddr, path, ns string, deploymentNN types.NamespacedName,
	suite *suite.ConformanceTestSuite, observedPods sets.Set[string],
) {
...
func podsAndDeploymentFullyReady(t *testing.T, pods []corev1.Pod, deployment appsv1.Deployment, observedPods sets.Set[string]) bool {
	for _, pod := range pods {
		...
		if pod.DeletionTimestamp != nil {
			tlog.Logf(t, "a pod is in deletion: %s DeletionTimestamp=%s", pod.Name, pod.DeletionTimestamp)
			return false
		}
	}
...
func awaitPodState(t *testing.T, threshold time.Duration, gwAddr, path string, client *nethttp.Client, description string, condition func(podState CapturedServerState) bool) { // nolint: unparam
...
func makeRequestForPodState(gwAddr, path string, client *nethttp.Client) (cReq CapturedServerState, err error) {
...
func makeRequest(t *testing.T, gwAddr, path, ns string, suite *suite.ConformanceTestSuite) (*roundtripper.CapturedRequest, error) {
...
func makeRequestEventually(t *testing.T, gwAddr, path, ns string, suite *suite.ConformanceTestSuite) { // nolint: unparam
